...
/* Date Input - Date Picker */
  ...
  // For mobile browsers, change the date picker inputs to type date in order to force the native UI on the phone/tablet
  if (/Mobi/.test(navigator.userAgent)) {
    $('[data-datepicker-trigger] input').attr('type', 'date');
    $('[data-datepicker-trigger] button').on('click', function(e){
      $(e.target).siblings('input').get(0).focus();
    });
  } else {
    initDatePicker();
  }
// Notes: Files that are uploaded should be added to FileList, remove should remove them from this list. FileList is readonly
// and therefore can't be manipulated. The solution is to create an array that is manipulated throughout this process and
// then on submit of the form the array is used to create the final FileList object before submission. fileStore is the array
// created to hold the file objects for each upload instance.
...
var vauxFileUpload = {
  ...
  bindUIFunctions: function(uploadObject, fileStore) {
    ...
    $(uploadObject).on('keydown', '.vaux-file-upload-wrapper span[role=button]', function(event) {
...
var vauxLoadMore = {
  ...
  bindUIfunctions: function(loadMoreObject) {
    ...
    loadMore.on('click touchstart', function (e) {
      var contentSections = $(loadMoreObject).find('section:hidden');
...
var vauxModal = {
  ...
  open: function(modal) {
    ...
    $(modal.modalId).on('keydown', function(event){
      vauxTrapKeyboard(event, this, {}, vauxModal.closeModal);
    });
...
function initPopover() {
    ...
    $('[data-toggle="popover"]')
        ...
        .click(function(e) {
            ...
            $('.vaux-popover-close').click(function(){
...
// Show only 1 label when screen is too small to fit multiple labels
function initProcessList() {
    $(window).on("resize", function () {
        $( ".vaux-process-list.has-bottom-label" ).each(function( ) {
            ...
            var bottomLabel = $('.process-bottom-label');

            // capture the Current step's text
            var currentStepText = $(this).find('ol .current span').text();
            ...
            $(this).find(bottomLabel).empty();

            // remove the class
            $(this).removeClass('has-bottom-label-activated');

            // If the container is too small to hold all Labels,
            // then show only the Bottom Label
            if (widthContainer <= widthLabels) {
                $(this).addClass('has-bottom-label-activated');

                // and add the Current step's text into it
                $(this).find(bottomLabel).append(currentStepText);
            }
        });
        // invoke this resize event immediately (on load)
    }).resize();
}
...
function initTimepicker() {
  $('[data-timepicker-trigger]').each(function(){
    var toggleInput = $(this).find('input');

    toggleInput.timepicker({
      appendWidgetTo: $(this)
    });
  });
}
...
// For mobile browsers, change the time picker inputs to type time in order to force the native UI on the phone/tablet
if (/Mobi/.test(navigator.userAgent)) {
  $('[data-timepicker-trigger] input').attr('type', 'time');
  $('[data-timepicker-trigger] button').on('click', function(e){
    $(e.target).siblings('input').get(0).focus();
  });
} else {
...
function validationDemo() {
    ...
    $('button.show-multiple-errors').click(function(event) {
        ...
        $('.validation-demo .usa-alert ul.usa-checklist').empty().append(
            '<li>The <a tabindex="0" href="#systolic">Systolic</a> value is outside the expected range.</li>\n' +
            '<li><a tabindex="0" href="#diastolic">Diastolic</a> should be less than <a>Systolic</a>. Please check the values.</a></li>\n' +
            '<li><a tabindex="0" href="#pulse">Pulse</a> field is required.</li>'
        );
